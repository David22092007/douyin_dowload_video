/**
 * Douyin_download_all_share_urls
 * Extracts share URLs (https://www.douyin.com/video/...) from a Douyin user profile
 */
...
const run = async function() {
    try {
        ...
        console.log(`Loading share URLs from user: ${sec_user_id}`);
        let max_cursor = 0;
        let errorCount = 0;
        ...
        while (hasMore == 1 && errorCount < 5) {
            try {
                ...
                for (const video of moredata.aweme_list) {
                    ...
                    if (video.share_info && video.share_info.share_url) {
                        shareUrl = video.share_info.share_url;
                    ...
                    console.clear();
                    console.log(`Collected: ${result.length} share URLs`);
                }
        ...
        if (result.length > 0) {
            console.log(`Saving ${result.length} share URLs to file...`);
            saveToFile(result.join('\n'));
            console.log(`Complete! Saved ${result.length} share URLs.`);
        } else {
            console.log("No share URLs found or unable to extract.");
        }
